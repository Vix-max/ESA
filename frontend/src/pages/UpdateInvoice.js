...
export default function UpdateInvoice() {
    ...
    const { id } = useParams(); // Extracts 'id' from the URL
      const navigate = useNavigate();
        ...
        const [invoice, setInvoice] = useState(null); // Add this state
        ...
        useEffect(() => {
            const fetchInvoice = async () => {
              try {
                const response = await fetch(`http://localhost:8000/api/getinvoicebyid/${id}`);
                if (response.ok) {
                  const data = await response.json();
                  setInvoice(data.invoice);
                  setCustomerName(data.invoice.customer_name);
                  setInvoiceDate(data.invoice.invoice_date);
          
                  // Populate entries with detailed item and variant data
                  const updatedEntries = await Promise.all(
                    data.invoice.entries.map(async (entry) => {
                      const itemResponse = await fetch(`http://localhost:8000/api/getitembyid/${entry.item}`);
                      const itemData = await itemResponse.json();
          
                      const variantResponse = await fetch(`http://localhost:8000/api/getvariantbyid/${entry.variant}`);
                      const variantData = await variantResponse.json();
          
                      return {
                        ...entry,
                        item: itemData.item.id, // Set the selected item ID
                        items: [itemData.item], // Populate the items array
                        variantId: variantData.variant.id, // Set the selected variant ID
                        variants: [variantData.variant], // Populate the variants array
                        marketPrice: variantData.variant.market_price || 0, // Set market price
                        buyPrice: variantData.variant.average_cost || 0, // Set cost price
                        sellPrice: variantData.variant.sell_price || 0, // Set sell price
                        stockAmount: variantData.variant.stock_amount || 0, // Set stock amount
                      };
                    })
                  );
    ...
    useEffect(() => {
      ...
      const handleUpdateItem = (item, variant) => {
        const newRow = {
  ...
  return (
    <div>
      <div>
      <div className="update-invoice-entry">
      
      <div className="update-invoice-header">
        <h1>Update Invoice</h1>
        <p>Invoices \ Update Invoice</p>
      </div>
      

      <form className="update-invoice-entry-form">
      <div ref={searchContainerRef} className="invoice-update-header">
    ...
    <div key={item.id} className="invoice-search-dropdown-item">
        {item.variants.map((variant) => (
            <div
                ...
                onClick={() => handleUpdateItem(item, variant)}
            >
                {`${variant.item_name}`}
            </div>
        ))}
            ...
            <div className="invoice-update-customer-info">
            ...
            <div className="invoice-update-date-info">
            <label>Date</label>
            <input
              type="date"
              value={invoiceDate}
            ...
            <table className="update-invoice-entry-table">
  <thead>
    <tr>
      <th>Category</th>
      <th>Brand</th>
...
>
  ...
  {(entry.variants || []).map((variant) => {
    ...
    <tfoot>
      <tr>
        <td colSpan="11">
          <div className="invoice-update-table-line"></div>
        </td>
      </tr>
    ...
    <tr>
      ...
      <td colSpan="2">
        <input
          type="number"
          className="update-invoice-profit"
          value={
            isNaN(
              invoiceEntries.reduce(
                (total, entry) =>
    ...
    <tr>
      ...
      <td colSpan="2">
        <input
          type="number"
          className="update-invoice-total"
          value={
            isNaN(
              invoiceEntries.reduce(
        ...
        <div className="invoice-update-remove-items">
          <button type="button" onClick={handleUpdateRow} className="invoice-update-item-button">
          Add
          </button>
        </div>
        <div className="update-invoice-actions">
        <button 
          type="button" 
          className="update-invoice-submit"
          onClick={handleGenerateInvoice}
        >
          Update Invoice
        </button>;
          <button type="button" className="update-invoice-back" onClick={() => navigate("/admin-dashboard/invoices")}>
            Back
          </button>
        </div>
      </form>
